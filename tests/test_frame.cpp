...
// the pair crossed: libwayland's dispatch reaches `operator new(nothrow)` —
// ASan's, which allocates from ASan's heap — and destruction then calls the
// sized `operator delete` here, which hands that pointer to free(). ASan reports
// it as alloc-dealloc-mismatch and aborts, which looks like a fault in the
// library and is nothing of the kind. So every form goes through the same
// malloc/free pair, and every allocating one is counted.
namespace {
