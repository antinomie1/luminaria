...
| shell | **CPU 合成器 `CpuCompositor`** —— 无 GPU 的通用合成路径（headless / 嵌套 / 测试）：把一棵表面树（含 subsurface）+ 混成器自己的纯色矩形按 z 序混合进 CPU RGBA buffer，处理 buffer scale / transform / viewport，预乘 alpha source-over；与 `Frame` 同一种立即模式图元列表（`CpuItem` = `RectFill` \| `CpuView` \| `CpuImage`），每个 `CpuView` 带设备坐标 clip（空 = 整帧），树内所有 surface 裁到它与 framebuffer 的交集，clip 外像素原样保留，图元身份是代际 `SurfaceId`，误留的列表也安全。按 surface 逻辑尺寸绘制，故输出级 scale/transform 仅 scale=1 / normal 一致（GPU 路径的 `Frame` 负责输出缩放旋转） | cpu_compositor |
...
| shell | **场景表 `SceneItem` / `SceneRenderer`** —— 一张有序列表同时被画和被命中：四种图元（客户端表面树、纯色矩形、窗口边框、混成器自有像素块），`SceneRenderer` 按现有硬件喂给 `Frame` 或 `CpuCompositor`，`scene_hit_test()` 读同一张表回答"这点上是什么"。上层因此每种图元只写一遍，不必维护 `Placement` / `CpuItem` 两套实现；三种降级（无 Vulkan 设备、输出配不出渲染目标、`submit()` 答 `fallback`）全在运行时落到同一个 CPU 分支。`tag` 是调用方身份，库不解释、命中时原样交还 | scene, scene_renderer |
