...
| shell | **CPU 合成器 `CpuCompositor`** —— 无 GPU 的通用合成路径（headless / 嵌套 / 测试）：把一棵表面树（含 subsurface）+ 混成器自己的纯色矩形按 z 序混合进 CPU RGBA buffer，处理 buffer scale / transform / viewport，预乘 alpha source-over；与 `Frame` 同一种立即模式图元列表（`CpuItem` = `RectFill` \| `CpuView`），每个 `CpuView` 带设备坐标 clip（空 = 整帧），树内所有 surface 裁到它与 framebuffer 的交集，clip 外像素原样保留，图元身份是代际 `SurfaceId`，误留的列表也安全。按 surface 逻辑尺寸绘制，故输出级 scale/transform 仅 scale=1 / normal 一致（GPU 路径的 `Frame` 负责输出缩放旋转） | cpu_compositor |
