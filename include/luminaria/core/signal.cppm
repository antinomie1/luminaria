// luminaria/core/signal.cppm — type-safe observer signal with RAII connections.
//
...
export module luminaria:core.signal;
...
class Signal {
    ...
    // Deliberately not make_shared: its combined allocation instantiates
    // std::type_info machinery, which a module interface unit would then have to
    // drag <typeinfo> in for at every point of instantiation.
    std::shared_ptr<Impl> impl_{new Impl};
