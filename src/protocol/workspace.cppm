...
// Lifted out of the anonymous namespace below: GroupInfo names it in a
...
// Lifted out of the anonymous namespace below: WorkspaceInfo names it in a
// member signature, and a module-linkage declaration may not expose a
// TU-local type. Still unexported — private to module luminaria.
...
using WsBinding = WorkspaceManager::Impl::Binding;
...
void manager_commit(wl_client*, wl_resource* resource) {
    auto* b = static_cast<WsBinding*>(wl_resource_get_user_data(resource));
...
void manager_resource_destroy(wl_resource* resource) {
    ...
    std::erase_if(b->impl->bindings,
                  [b](const std::unique_ptr<WsBinding>& held) { return held.get() == b; });
}
...
void manager_bind(wl_client* client, void* data, uint32_t version, uint32_t id) {
    ...
    auto binding = std::make_unique<WsBinding>();
    binding->resource = resource;
    binding->impl = impl;
    WsBinding* b = binding.get();
...
void WorkspaceManager::Impl::send_group(WsBinding& b, const GroupInfo& g) {
...
void WorkspaceManager::Impl::send_group_outputs(WsBinding& b, const GroupInfo& g) {
...
void WorkspaceManager::Impl::send_workspace(WsBinding& b, const WorkspaceInfo& w) {
    wl_client* client = wl_resource_get_client(b.resource);
