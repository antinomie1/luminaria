...
using Mgr = DataControlManager::Impl;
...
public:
    ControlSource(Mgr* mgr, wl_resource* resource) : mgr_(mgr), resource_(resource) {}
...
void invalidate_offers(Mgr* mgr, bool primary) {
...
void send_offer_to_device(Mgr* mgr, wl_resource* device, const std::vector<std::string>& mimes,
                          bool primary) {
...
void broadcast(Mgr* mgr, const std::vector<std::string>& mimes, bool primary) {
