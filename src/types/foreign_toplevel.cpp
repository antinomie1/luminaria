...
using Impl = ForeignToplevelManager::Impl;
using Entry = Impl::Entry;
using Binding = Impl::Binding;
...
Entry& Impl::track_toplevel(Toplevel& toplevel) {
    if (Entry* existing = entry_for(&toplevel); existing != nullptr) {
        return *existing;
    }
...
void Impl::announce(Entry& entry) {
    if (entry.announced) {
        return;
    }
...
void Impl::withdraw(Entry& entry) {
    if (!entry.announced) {
        return;
    }
...
void Impl::forget(Entry& entry) {
    withdraw(entry);
    std::erase_if(entries, [&entry](const std::unique_ptr<Entry>& e) { return e.get() == &entry; });
}

void Impl::send_handle(Binding& binding, Entry& entry) {
...
/// same output twice.
void Impl::send_details(Binding& binding, Entry& entry) {
...
void Impl::send_state(wl_resource* handle, const Entry& entry) {
    wl_array states;
...
void Impl::send_parent(Binding& binding, wl_resource* handle, const Entry& entry) {
    if (wl_resource_get_version(handle) <
        ZWLR_FOREIGN_TOPLEVEL_HANDLE_V1_PARENT_SINCE_VERSION) {
        return;
    }
    wl_resource* parent_handle = nullptr;
    if (Entry* parent = entry_for(entry.toplevel->parent()); parent != nullptr) {
...
ForeignToplevelManager::ForeignToplevelManager(std::unique_ptr<Impl> impl) noexcept
    : impl_(std::move(impl)) {}
...
void ForeignToplevelManager::set_output(Toplevel& toplevel, OutputGlobal* output) {
    Impl::Entry* entry = impl_->entry_for(&toplevel);
